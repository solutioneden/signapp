...
(function () {
    ...
    app.run(['$rootScope', function($rootScope){
        $rootScope.serverUrl = "http://192.168.168.25:8080";
    }]);

    app.controller('listSignInController', function ($rootScope, $scope, $http, $timeout) {

        $http.get($rootScope.serverUrl+'/people/listSignIn?isMobile=true').
        ...
        $scope.signOut = function(id) {
            $.post($rootScope.serverUrl+"/people/signOut", {id: id})
                .done(function(data) {
                    $timeout(function(){
        ...
        $scope.repeatSignIn = function(id) {
            $.post($rootScope.serverUrl+"/people/clearSignOut", {id: id})
                .done(function(data) {
                    $timeout(function(){
                        menu.setMainPage('index.html', {closeMenu: true});
                    }, 1000);
    ...
    app.controller('listStaffSignInController', function ($rootScope, $scope, $http, $timeout) {

        $http.get($rootScope.serverUrl+'/people/listStaffs?isMobile=true').
            success(function (data, status, headers, config) {
                $scope.signins = data;
                $scope.serverUrl = $rootScope.serverUrl;
            }).
    ...
    app.controller('signAppController', function ($rootScope, $http, $scope, $timeout) {
        $scope.signIn = function() {
            $.post($rootScope.serverUrl + "/people/signIn", $("#signInForm").serialize())
                .done(function(data) {
                    $timeout(function(){
                        menu.setMainPage('index.html', {closeMenu: true});
                    }, 1000);
